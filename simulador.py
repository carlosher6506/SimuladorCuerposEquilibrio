...
class Simulador:
    def __init__(self, width, height):
        ...
        self.theta1 = 45  # Inicializando theta1
        self.theta2 = 45  # Inicializando theta2
        self.weight = 100  # Valor predeterminado para el peso
        self.g = 9.81  # Valor de la gravedad
        self.block_pos = [400, 300]
        
        # Cargar imágenes
        self.load_images()

    def load_images(self):
        self.rotacion_image = pygame.image.load(os.path.abspath('Giro.png'))
        self.peso_image = pygame.image.load(os.path.abspath('Peso.png'))
        self.polea_image = pygame.image.load(os.path.abspath('Poleas2.png'))
        self.rope_image = pygame.image.load(os.path.abspath('1.png'))
        self.fondo_image = pygame.image.load(os.path.abspath('Fondo3.png'))
    ...
    def solution(self):
        ...
        T22 = self.weight / (math.sin(theta2_rad) + math.cos(theta2_rad) * math.tan(theta1_rad))
        T11 = self.weight / (math.sin(theta1_rad) + math.cos(theta1_rad) * math.tan(theta2_rad))
        return T11, T22
    ...
    def calculate_tensions(self):
        ...
        T2 = self.weight * math.sin(theta1_rad) / denominator
        T1 = self.weight * math.sin(theta2_rad) / denominator

        return T1, T2

    def calculate_body_position(self, anchor1_x, anchor2_x, anchor_y, T1, T2):
        denominator = (T1 * math.cos(math.radians(self.theta1)) + T2 * math.cos(math.radians(self.theta2)))

        if denominator == 0:
            return anchor1_x, anchor_y  # Evitar división por cero

        body_x = (T2 * math.cos(math.radians(self.theta2)) * anchor1_x + 
                  T1 * math.cos(math.radians(self.theta1)) * anchor2_x) / denominator
        body_y = anchor_y + T1 * math.sin(math.radians(self.theta1))
        return int(body_x), int(body_y)

    def draw_scene(self, screen, block_pos, theta1, theta2, result_conversion, conversor_visible):
        if theta1 is None:
            theta1 = self.theta1
        if theta2 is None:
            theta2 = self.theta2
        ...
        T1, T2 = self.calculate_tensions(weight,theta1,theta2)
        T11, T22 = self.solution(self.weight, self.theta1, self.theta2)
        ...
        self.screen.blit(fondo_image, fondo_image_rect)
        ...
        self.screen.blit(peso_image, body_image_rect)
        ...
        self.screen.blit(texto_grafico, (260, 835))
        ...
        if mostrar_grafico and superficie_grafico:
            ...
            # Dibujar el gráfico
                self.screen.blit(superficie_grafico, (grafico_x, grafico_y))
    
    def run(self):
        running = True
        conversor = Conversor()
        result_conversion = 0
        conversor_visible = False
        dragging = False  # Inicializamos la variable dragging aquí
        mouse_offset = [0, 0]
        ...
        while running:
            ...
            self.theta1 = max(0, min(self.theta1, 90))  # Cambia theta1 por self.theta1
            self.theta2 = max(0, min(self.theta2, 90))  # Cambia theta2 por self.theta2
            ...
            # Dibujar la escena
            self.draw_scene(self.screen, self.block_pos, self.theta1, self.theta2, result_conversion, conversor_visible)
            pygame.display.flip()
            ...
            self.draw_scene(result_conversion=result_conversion, conversor_visible=conversor_visible)
            pygame.display.flip()
